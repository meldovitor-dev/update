...
export class LoginComponent implements OnInit, AfterContentInit {
  ...
  logoutAndDismiss(data?: any) {
    const isLoggedIn = !!this.store.selectSnapshot(UserState.getUser).authorization;
    if (isLoggedIn) {
  ...
  async login() {
    ...
    if (!connected) {
      this.setCurrentPage(GET_LOGIN_ERROR_PAGE(LOGIN_ERROR_PAGE_CATALOG.offline));
      return;
    }
    this.refreshUserObj();
          ...
          this.loginService
            ...
            .subscribe(
              ...
              err => {
                ...
                if (statusText === 'Unknown Error' || status === 500 || status === 503) {
  ...
  public callNovaFibraChecker(params) {
    ...
    this.loginService.checkNovaFibra(cpfOrCnpj).subscribe((res: any) => {
  ...
  public handlerNovaFibraUser() {
    ...
    if (isAndroid){
      this.setCurrentPage(GET_LOGIN_ERROR_PAGE(
